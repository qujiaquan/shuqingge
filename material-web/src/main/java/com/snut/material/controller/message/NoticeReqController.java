...
@RestController
@RequestMapping(path = "/admin/noticeReqCtl")
public class NoticeReqController {
     ...
     * @return
     */
    @GetMapping(path = "/noticeReq/{num}/{pageSize}")
     ...
     * @return
     */
    @GetMapping(path = "/findNoticeReq/{account}/{num}/{pageSize}")
    public CommonResult findNoticeReq(@PathVariable("account") String account, @PathVariable("num") Integer num, @PathVariable("pageSize") Integer pageSize) {
        CommonResult commonResult = null;
        PageInfo<NoticeReqEntity> pageInfo = noticeReqService.findNoticeReq(account, num, pageSize);
    ...
    public CommonResult changeState(@PathVariable("row") Integer row, @PathVariable("flag") Integer flag, @RequestHeader("adminToken") String adminToken) {
        CommonResult commonResult = null;
        noticeReqService.changeState(row, flag);
    ...
    public CommonResult saveNoticeReq(@RequestBody NoticeReqEntity noticeReq, @RequestHeader("adminToken") String adminToken) {
        CommonResult commonResult = null;
        if (noticeReq.getId() == null) {
            noticeReqService.saveNoticeReq(noticeReq);
        } else {
            noticeReqService.update(noticeReq, adminToken);
        }
     ...
     * @return
     */
    @GetMapping(path = "/findNoticeReqByid/{id}")
    public CommonResult findNoticeReqByid(@PathVariable("id") Integer id) {
        CommonResult commonResult = null;
        NoticeReqEntity noticeReq = noticeReqService.findNoticeReqByid(id);
        commonResult = new CommonResult(200, noticeReq, "查询成功");
